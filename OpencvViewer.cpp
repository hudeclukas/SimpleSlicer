...
void CV_Viewer::mouseCallBackFunction(int event, int x, int y, int flags, void* data)
{
    ...
    if (flags==CV_EVENT_FLAG_LBUTTON)
    {
        ...
        cv::Mat slice = cv::Mat::zeros(size_, size_, CV_8UC3);
        std::vector<cv::Vec3i> hst(256);
        cv::Vec3i maxcounts = { 0,0,0 };

        for (auto row=0; row < size_; ++row)
        {
            auto imgrow = image->ptr<cv::Vec3b>(y <= shift ? row : y + shift <= image->rows ? y - shift + row : image->rows - size_ + row);
            auto slicerow = slice.ptr<cv::Vec3b>(row);
            for (auto col = 0; col < size_; ++col)
            {
                auto pixel = slicerow[col] = imgrow[x <= shift ? col : x + shift <= image->cols ? x - shift + col : image->cols - size_ + col];
                if (++hst[pixel[0]][0] > maxcounts[0]) maxcounts[0]++;
                if (++hst[pixel[1]][1] > maxcounts[1]) maxcounts[1]++;
                if (++hst[pixel[2]][2] > maxcounts[2]) maxcounts[2]++;
            }
        }

        cv::Mat histogram = cv::Mat::zeros(200, 256, CV_8UC3);
        ...
        cv::rectangle(*image, cv::Point(x < shift ? 0 : x - shift, y < shift? 0 : y - shift), cv::Point(x + shift > image->cols ? image->cols : x + shift, y + shift > image->rows ? image->rows : y + shift), cv::Scalar(0, 255, 0), 4, cv::LINE_8, 0);
...
void mouseCallBackFunction1(int event, int x, int y, int flags, void* data)
{
    ...
    //TODO verify the stability of local variable sent in signal
